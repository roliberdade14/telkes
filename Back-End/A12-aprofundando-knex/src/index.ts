...
app.get("/bands", async (req: Request, res: Response):Promise<void> => {
    try {
        ...
        //const result:Tbands[] = await db.select('*').from('bands')
        const result:Tbands[] = await db('bands')
...
app.delete("/bands/:id", async (req: Request, res: Response):Promise<void>=>{
    ...
    try{
        ...
        res.status(200).send({message: "Banda excluida com sucesso!"})
